...
export default class CustomTabBar extends Component{
    ...
    renderTabOption(tab,i){
        let color = this.props.activeTab === i ? "#1296db" : "#707070";//判断i是否为当前选中的tab,设置不同的颜色
        let tabName = this.props.tabNames[i];

        return (
            <TouchableOpacity onPress={()=>this.props.goToPage(i)} style={[styles.tab]} key={'tab'+i}>
                <View style={[styles.tabBox]}>
                    <Image source={tabIcons[this.props.activeTab === i ?i*2 : i*2+1]} style={[styles.tabBoxIcon]}/>
                    <Text style={[styles.tabBoxName,{color:color}]}>{tabName}</Text>
                </View>
            </TouchableOpacity>
        )
    }
...
CustomTabBar.propTypes = {
    ...
    tabNames:PropTypes.array,//所有Tab名称
